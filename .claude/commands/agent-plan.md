...
3. 依級別派審查（全部 readonly；prompt 不得含兒童資料或金鑰）：
   - 工程審：`codex exec -m gpt-6-luna -s read-only -c model_reasoning_effort="medium" "<prompt>" </dev/null`。prompt 寫明「你未撰寫此 Plan」，逐條反駁 DAG，至少 3 點。
   - 對抗審（L3）：`cursor-agent -p --trust --mode ask --model grok-4.7-high-fast`；失敗時改用 `grok -m grok-4.7 --permission-mode plan -p "<prompt>"`。
   - 設計審（L3，或碰到設計審觸發項）：Agent tool `model: "opus"`（Opus 5.5，effort high）。
